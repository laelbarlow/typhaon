...
DIR=~/env_typhaon_snakemake_workflow
...
if test "$(command -v conda)"; then
  ...
  if ! test "$(conda info --envs | grep "conda_env_typhaon_snakemake_workflow")"; then
    #conda create --name conda_env_typhaon_snakemake_workflow python=3.7.4
    #source activate conda_env_typhaon_snakemake_workflow
    ...
    -n conda_env_typhaon_snakemake_workflow \
        snakemake \
        numpy \
        pandas \
        graphviz \
    ...
    echo Dependencies have been installed in a new environment.
    echo Now re-run to activate the environment. 
    exit 1
