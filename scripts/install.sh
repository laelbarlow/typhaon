...
PYENVDIR=~/env_typhaon_workflow_setup_py
...
if [ "$(uname)" == "Darwin" ]; then
    ...
    # SnakeMake profile.
    if [ "$snakemake_profile" == "slurm_typhaon" ]; then
        ...
        if [ ! -d ~/.config/snakemake/slurm_typhaon ]; then
            rm -rf ~/.config/snakemake/slurm_typhaon
        fi
        ...
        ~/.config/snakemake/slurm_typhaon/slurm-jobscript.sh

    elif [ "$snakemake_profile" == "sge_typhaon" ]; then
        ...
        # Removing existing directory, if present.
        if [ ! -d ~/.config/snakemake/sge_typhaon ]; then
            rm -rf ~/.config/snakemake/sge_typhaon
        fi
        ...
        cookiecutter https://github.com/Snakemake-Profiles/sge.git \
            profile_name="sge_typhaon"
        # Make Python scripts in profile executable.
        chmod a+x ~/.config/snakemake/sge_typhaon/*.py
        ...
        # Removing existing directory, if present.
        if [ ! -d ~/.config/snakemake/pbs-torque_typhaon ]; then
            rm -rf ~/.config/snakemake/pbs_torque_typhaon
        fi
        ...
        cookiecutter https://github.com/Snakemake-Profiles/pbs-torque.git \
            profile_name="pbs-torque_typhaon"
        # Make Python scripts in profile executable.
        chmod a+x ~/.config/snakemake/pbs-torque_typhaon/*.py
        # Change back to previous directory.
        cd -
        # Now you can run snakemake as: snakemake --profile pbs-torque_typhaon ...
